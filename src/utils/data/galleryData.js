// data/galleryData.js

export const galleryCategories = [
  { id: "all", label: "الكل" },
  { id: "suits", label: "بدلات عروس" },
  { id: "buffets", label: "بوفيهات" },
  { id: "birthday", label: "أعياد ميلاد" },
  { id: "arada", label: "عراضات شامية" },
  { id: "parties", label: "حفلات" },
  { id: "events", label: "مناسبات" },
];

export const galleryImages = [
  // بدلات عروس
  {
    id: 1,
    category: "suits",
    url: "https://plus.unsplash.com/premium_photo-1661340932540-c285e730728a?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    alt: "فستان زفاف أبيض فاخر",
    label: "فستان زفاف أبيض فاخر",
  },
  {
    id: 2,
    category: "suits",
    url: "https://images.unsplash.com/photo-1593575620619-602b4ddf6e96?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    alt: "فستان عروس أنيق بذيل طويل",
    label: "فستان عروس أنيق بذيل طويل",
  },
  {
    id: 3,
    category: "suits",
    url: "https://images.unsplash.com/photo-1594552072238-b8a33785b261?w=700&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjR8fHdlZGRpbmclMjBkcmVzc3xlbnwwfHwwfHx8MA%3D%3D",
    alt: "فستان زفاف",
    label: "فستان زفاف",
  },

  // بوفيهات
  {
    id: 4,
    category: "buffets",
    url: "https://images.unsplash.com/photo-1555244162-803834f70033?w=800",
    alt: "بوفيه فاخر",
    label: "بوفيه فاخر",
  },
  {
    id: 5,
    category: "buffets",
    url: "https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=800",
    alt: "مائدة طعام",
    label: "مائدة طعام",
  },
  {
    id: 6,
    category: "buffets",
    url: "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=800",
    alt: "حلويات",
    label: "حلويات",
  },

  // أعياد ميلاد
  {
    id: 7,
    category: "birthday",
    url: "https://images.unsplash.com/photo-1530103862676-de8c9debad1d?w=800",
    alt: "عيد ميلاد",
    label: "عيد ميلاد",
  },
  {
    id: 8,
    category: "birthday",
    url: "https://images.unsplash.com/photo-1464349153735-7db50ed83c84?w=800",
    alt: "كعكة عيد ميلاد",
    label: "كعكة عيد ميلاد",
  },
  {
    id: 9,
    category: "birthday",
    url: "https://images.unsplash.com/photo-1527529482837-4698179dc6ce?w=800",
    alt: "بالونات",
    label: "بالونات",
  },

  // عراضات شامية
  {
    id: 10,
    category: "arada",
    url: "https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3?w=800",
    alt: "عراضة شامية",
    label: "عراضة شامية",
  },
  {
    id: 11,
    category: "arada",
    url: "https://images.unsplash.com/photo-1504196606672-aef5c9cefc92?w=800",
    alt: "فرقة شامية",
    label: "فرقة شامية",
  },
  {
    id: 12,
    category: "arada",
    url: "https://images.unsplash.com/photo-1501281668745-f7f57925c3b4?w=800",
    alt: "احتفال تقليدي",
    label: "احتفال تقليدي",
  },

  // حفلات
  {
    id: 13,
    category: "parties",
    url: "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=800",
    alt: "حفلة",
    label: "حفلة",
  },
  {
    id: 14,
    category: "parties",
    url: "https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=800",
    alt: "حفلة موسيقية",
    label: "حفلة موسيقية",
  },
  {
    id: 15,
    category: "parties",
    url: "https://images.unsplash.com/photo-1429962714451-bb934ecdc4ec?w=800",
    alt: "سهرة",
    label: "سهرة",
  },

  // مناسبات
  {
    id: 16,
    category: "events",
    url: "https://images.unsplash.com/photo-1519741497674-611481863552?w=800",
    alt: "حفل زفاف",
    label: "حفل زفاف",
  },
  {
    id: 17,
    category: "events",
    url: "https://images.unsplash.com/photo-1511578314322-379afb476865?w=800",
    alt: "مناسبة خاصة",
    label: "مناسبة خاصة",
  },
  {
    id: 18,
    category: "events",
    url: "https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800",
    alt: "فعالية",
    label: "فعالية",
  },
];
