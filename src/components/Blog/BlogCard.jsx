...
export default function BlogCard({
  ...
  /* ══════════════════════════════════════════════
     Build href:
     /blog/news/{slug}?id={id}

     - slug for clean URL appearance
     - id in query for the detail API request
     ══════════════════════════════════════════════ */
  const detailHref = id ? `/blog/news/${slug}?id=${id}` : `/blog/news/${slug}`;
    ...
    >
      ...
      >
        ...
        <div className="flex flex-1 flex-col gap-2 px-5 pt-4 pb-5">
          ...
          {excerpt ? (
            <div className="line-clamp-3 flex-1 font-poppins! text-[12px] leading-relaxed text-secondary">
              <div
                className=""
                _dangerouslySetInnerHTML={{ __html: excerpt }}
              />
            </div>
          ) : (
